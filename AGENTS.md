...
- **Auth:** `PyJWT` (HS256 access tokens); Identity Toolkit REST for password login (see `identity_toolkit.py`).
...
fambot_backend/
  ...
  identity_toolkit.py # signInWithPassword for POST /v1/auth/login
...
| `FIREBASE_WEB_API_KEY` | Required for `POST /v1/auth/login` (Identity Toolkit). |
...
1. **Signup:** `POST /v1/auth/signup` creates a Firebase Auth user with `firebase_admin.auth.create_user`, ensures a Firestore user doc, mints a JWT (`sub` = Firebase `uid`).
2. **Login:** `POST /v1/auth/login` calls Identity Toolkit `signInWithPassword` (requires `FIREBASE_WEB_API_KEY`), then mints the same JWT shape from `localId`.
3. **Protected routes:** client sends `Authorization: Bearer <JWT>`. `deps.firebase_uid` verifies the JWT with `jwt_tokens.decode_and_verify` and returns `sub` as the uid string (name kept for minimal churn).
...
| Task | Primary files |
|------|----------------|
| New endpoint | `app.py`, possibly `schemas.py`; update **README** API section |
...
| Auth behavior | `deps.py`, `jwt_tokens.py`, `identity_toolkit.py`, `firebase_init.py` |
